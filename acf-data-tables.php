...
class ACF_Data_Tables
{
    ...
    public function render_shortcode_metabox($post)
    {
        ...
        <div class="acf-dt-shortcode-box">
            <input type="text" readonly value='[acf_table id="<?php echo $post->ID; ?>"]' onclick="this.select();"
                style="width: 100%;">
            <p class="description">Copy and paste into your post or page.</p>

            <p style="margin-top: 10px;"><strong>Additional Options:</strong></p>
            <code
                style="font-size: 11px; display: block; margin-top: 5px;">[acf_table id="<?php echo $post->ID; ?>" class="my-class"]</code>
        </div>
        <?php
    }
                        ...
                        <?php else: ?>
                            <?php foreach ($table_data as $row_index => $row): ?>
                                <tr data-row="<?php echo $row_index; ?>">
                                    ...
                                    <?php foreach ($columns as $col): ?>
                                        <td>
                                            <input type="text" class="acf-dt-cell" data-col="<?php echo esc_attr($col['col_key']); ?>"
    ...
    public function ajax_import_html()
    {
        ...
        // Define allowed HTML tags with their attributes for import
        $allowed_tags = $this->get_allowed_html_tags();

        $html = isset($_POST['html']) ? wp_kses($_POST['html'], $allowed_tags) : '';
        $post_id = isset($_POST['post_id']) ? intval($_POST['post_id']) : 0;
        ...
        foreach ($trs as $tr) {
            ...
            // Data row - preserve inner HTML
            $tds = $tr->getElementsByTagName('td');
            if ($tds->length > 0) {
                $row = [];
                foreach ($tds as $td) {
                    // Get the inner HTML of the cell to preserve tags
                    $row[] = $this->get_inner_html($td, $dom);
                }
                $data_rows[] = $row;
            }
        }
        ...
        // Build columns and detect which ones contain HTML
        $column_has_html = [];
        $columns = [];
        foreach ($header_row as $index => $label) {
            ...
            $columns[] = [
                'col_key' => $key,
                'col_label' => $label,
                'col_type' => 'text', // Will be updated if HTML is detected
                'col_align' => 'left',
                'col_width' => '',
            ];
            $column_has_html[$index] = false;
        }

        // Build rows and detect HTML content
        $rows = [];
        foreach ($data_rows as $data_row) {
            $row_data = [];
            foreach ($columns as $index => $col) {
                $cell_value = isset($data_row[$index]) ? trim($data_row[$index]) : '';
                $row_data[$col['col_key']] = $cell_value;

                // Check if this cell contains HTML tags
                if (!$column_has_html[$index] && $this->contains_html($cell_value)) {
                    $column_has_html[$index] = true;
                }
            }
            $rows[] = ['row_data' => json_encode($row_data, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)];
        }

        // Update column types for columns that contain HTML
        foreach ($column_has_html as $index => $has_html) {
            if ($has_html) {
                $columns[$index]['col_type'] = 'html';
            }
        }
    ...
    /**
     * Get allowed HTML tags for import and display
     * Focuses on preserving classes, urls (href/src), and inline styles
     */
    private function get_allowed_html_tags()
    {
        $allowed = wp_kses_allowed_html('post');

        // Ensure standard attributes are preserved for key tags
        $tags = ['a', 'span', 'img', 'strong', 'b', 'em', 'i', 'tr', 'td', 'th', 'table', 'thead', 'tbody', 'tfoot'];
        ...
        foreach ($tags as $tag) {
            ...
            if ($tag === 'img') {
                ...
                $allowed[$tag]['srcset'] = true;
                $allowed[$tag]['sizes'] = true;
            }
     ...
     */
    private function get_inner_html($element, $dom)
    {
        $inner_html = '';
    ...
    {
        // Check for common HTML tags that we want to preserve
        return preg_match('/<(a|span|img|strong|b|em|i|br)[^>]*>/i', $string) === 1;
    }
    ...
    public function render_shortcode($atts)
    {
        ...
        if ($striped)
            $classes[] = 'acf-dt-striped';
        if ($hover)
            $classes[] = 'acf-dt-hover';
        if ($sortable)
            $classes[] = 'acf-dt-sortable';
        if ($searchable)
            $classes[] = 'acf-dt-searchable';
        if ($custom_class)
    ...
    private function format_cell_value($value, $type)
    {
        switch ($type) {
            ...
            case 'html':
                return wp_kses($value, $this->get_allowed_html_tags());
